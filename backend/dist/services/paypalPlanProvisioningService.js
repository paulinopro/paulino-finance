...
exports.provisionPaypalSubscriptionPlan = provisionPaypalSubscriptionPlan;
exports.syncPaypalSubscriptionPlanById = syncPaypalSubscriptionPlanById;
...
async function paypalJson(method, path, body) {
    const token = await (0, paypalService_1.getAccessToken)();
    if (!token) {
...
async function createCatalogProduct(name, description) {
    ...
    const r = await paypalJson('POST', '/v1/catalogs/products', body);
    if (!r.ok)
        return { error: `Producto PayPal: ${r.status} ${r.text}` };
    if (!r.data.id)
        return { error: 'PayPal no devolvió id de producto' };
    return { id: r.data.id };
}
async function createBillingPlan(params) {
    ...
    const r = await paypalJson('POST', '/v1/billing/plans', body);
    if (!r.ok)
        return { error: `Plan billing PayPal: ${r.status} ${r.text}` };
    if (!r.data.id)
        return { error: 'PayPal no devolvió id de plan' };
 ...
 */
async function activateBillingPlanIfNeeded(planId, statusAfterCreate) {
    const st = (statusAfterCreate || '').toUpperCase();
    if (st === 'ACTIVE') {
        return {};
    }
    const r = await paypalJson('POST', `/v1/billing/plans/${encodeURIComponent(planId)}/activate`);
    if (r.ok)
        return {};
...
function isPlanAlreadyActiveError(body) {
    try {
 ...
 */
async function provisionPaypalSubscriptionPlan(row) {
    ...
    if (!paypalPlanIdMonthly) {
        ...
        if (act.error)
            return { ok: false, message: act.error };
...
async function syncPaypalSubscriptionPlanById(planId) {
    const r = await (0, database_1.query)(`SELECT id, name, description, price_monthly, price_yearly, currency,
    ...
    if (!result.ok)
        return result;
    await (0, database_1.query)(`UPDATE subscription_plans
     SET paypal_product_id = $1,
         paypal_plan_id_monthly = $2,
         paypal_plan_id_yearly = $3,
         updated_at = CURRENT_TIMESTAMP
     WHERE id = $4`, [result.paypalProductId, result.paypalPlanIdMonthly, result.paypalPlanIdYearly, planId]);
    return result;
}
//# sourceMappingURL=paypalPlanProvisioningService.js.map