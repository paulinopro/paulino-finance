...
describe('agenda synchronization orchestration', () => {
    ...
    it('keeps a failed outbound item pending and reports the provider error', async () => {
        mockQuery.mockImplementation(async (sql) => {
            ...
            if (text.includes('SELECT provider, import_from, import_to'))
                return { rows: [] };
            if (text.includes('SELECT id, deleted_at'))
                return { rows: [{ id: 41, deleted_at: null }] };
            return { rows: [] };
        });
        ...
        const syncedWrite = mockQuery.mock.calls.find(([sql]) => normalized(sql).includes("sync_status = 'SYNCED'"));
    ...
    it('imports only the selected provider with the requested range', async () => {
        mockQuery.mockImplementation(async (sql) => {
            const text = normalized(sql);
            if (text.startsWith('UPDATE agenda_provider_connections SET'))
                return { rows: [{ id: 12 }] };
            if (text.includes('SELECT id, deleted_at'))
                return { rows: [] };
            return { rows: [] };
        });
        await syncAgendaImportRange(7, 'ICLOUD_CALDAV', '2024-01-01T00:00:00.000Z', '2028-01-01T00:00:00.000Z');
...
//# sourceMappingURL=agendaSyncService.test.js.map