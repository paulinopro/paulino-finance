         ...
         WHERE user_id = $1 AND enabled = true`, [userId]);
           ...
           WHERE user_id = $1`, [userId]);
                for (const card of cardsResult.rows) {
                 ...
                 ON CONFLICT DO NOTHING`, [userId, title.replace(/<[^>]*>/g, ''), message.replace(/<[^>]*>/g, ''), card.id]);
           ...
           WHERE user_id = $1 AND status = 'ACTIVE'`, [userId]);
                for (const loan of loansResult.rows) {
                 ...
                 ON CONFLICT DO NOTHING`, [userId, title.replace(/<[^>]*>/g, ''), message.replace(/<[^>]*>/g, ''), loan.id]);
                            await (0, telegramService_1.sendTelegramMessage)(telegramChatId, message);
                        }
                    }
                }
           ...
           WHERE user_id = $1 
             ...
             AND (last_paid_month IS NULL 
                ...
                for (const expense of expensesResult.rows) {
                 ...
                 ON CONFLICT DO NOTHING`, [userId, title.replace(/<[^>]*>/g, ''), message.replace(/<[^>]*>/g, ''), expense.id]);
                            await (0, telegramService_1.sendTelegramMessage)(telegramChatId, message);
                        }
                    }
                }
