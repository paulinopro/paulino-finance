...
exports.updateUserAdmin = exports.stopImpersonation = exports.impersonateUser = exports.updateSystemSettings = exports.getSystemSettings = exports.listUsers = void 0;
...
const listUsers = async (req, res) => {
    try {
        ...
        const searchWhereU = 'WHERE u.email ILIKE $1 OR COALESCE(u.first_name,\'\') ILIKE $1 OR COALESCE(u.last_name,\'\') ILIKE $1';
        const countResult = search
            ? await (0, database_1.query)(`SELECT COUNT(*)::int AS c FROM users u ${searchWhereU}`, [pattern])
            : await (0, database_1.query)(`SELECT COUNT(*)::int AS c FROM users`);
        ...
        const result = search
            ? await (0, database_1.query)(`${baseSelect}
           ${baseJoin}
           ${searchWhereU}
...
const impersonateUser = async (req, res) => {
    try {
        ...
        const target = await (0, database_1.query)(`SELECT id, email, first_name, last_name, telegram_chat_id, currency_preference,
...
const stopImpersonation = async (req, res) => {
    try {
        ...
        const admin = await (0, database_1.query)(`SELECT id, email, first_name, last_name, telegram_chat_id, currency_preference,
              exchange_rate_dop_usd, timezone, is_super_admin
       FROM users WHERE id = $1 AND is_super_admin = true`, [impersonatedBy]);
