...
function amountForVariableMonthlyIncomeRow(row, receivedThisMonth, monthlyRec) {
    const base = parseFloat(String(row.amount));
    if (!monthlyRec || row.nature !== 'variable' || !receivedThisMonth)
        return base;
...
function incomeDisplayedReceived(row, currentMonth, currentYear) {
...
const createIncome = async (req, res) => {
    ...
    const monthlyReceivedInitial = initialReceived && recurrenceType === 'recurrent' && freqNorm === 'monthly';
    ...
    try {
       ...
       RETURNING id, description, amount, currency, nature, recurrence_type, frequency,
        ...
        if (monthlyReceivedInitial && nature === 'variable') {
            await (0, recurringPeriodAmounts_1.upsertIncomePeriodAmount)(client, userId, newId, currentYear, currentMonth, amt);
        }
...
function resolveMonthlyIncomeReceiptAmount(row, monthlyRec, actualAmountRaw) {
    const base = parseFloat(String(row.amount));
    if (!monthlyRec || row.nature !== 'variable')
        return base;
...
const updateIncomeReceiptStatus = async (req, res) => {
    try {
        ...
        const checkResult = await (0, database_1.query)(`SELECT is_received, description, bank_account_id, amount, currency, nature, recurrence_type, frequency,
       ...
       FROM income WHERE id = $1 AND user_id = $2`, [incomeId, userId]);
        ...
        try {
           ...
           RETURNING id, is_received, updated_at`, [isReceived, incomeId, userId]);
            ...
            const alreadyReceivedThisPeriod = row.last_received_month === currentMonth &&
                row.last_received_year === currentYear &&
                Boolean(row.is_received);
            if (isReceived) {
                if (row.nature === 'variable') {
                    await (0, recurringPeriodAmounts_1.upsertIncomePeriodAmount)(client, userId, incomeId, currentYear, currentMonth, receiptAmt);
                }
                if (row.bank_account_id && !alreadyReceivedThisPeriod) {
                    try {
                        await (0, accountBalance_1.applyBalanceDelta)(userId, row.bank_account_id, row.currency, receiptAmt, client, {
           ...
           RETURNING id, is_received, updated_at`, [currentMonth, currentYear, incomeId, userId]);
                await client.query('COMMIT');
                const r = result.rows[0];
                res.json({
                    success: true,
                ...
                if (row.nature === 'variable') {
                    const stored = await (0, recurringPeriodAmounts_1.getIncomePeriodAmount)(userId, incomeId, currentYear, currentMonth, client);
                    if (stored != null)
                        reverseAmt = stored;
                }
                if (displayedReceived && row.bank_account_id) {
                    try {
                        await (0, accountBalance_1.applyBalanceDelta)(userId, row.bank_account_id, row.currency, -reverseAmt, client, {
                ...
                if (row.nature === 'variable') {
                    await (0, recurringPeriodAmounts_1.deleteIncomePeriodAmount)(client, userId, incomeId, currentYear, currentMonth);
                }
                const result = await client.query(`UPDATE income
           SET is_received = false, updated_at = CURRENT_TIMESTAMP
           WHERE id = $1 AND user_id = $2
           RETURNING id, is_received, updated_at`, [incomeId, userId]);
                await client.query('COMMIT');
                const r = result.rows[0];
                res.json({
                    success: true,
