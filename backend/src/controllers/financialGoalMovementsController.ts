...
function runQuery(client: PoolClient | undefined, text: string, params?: unknown[]) {
...
async function syncGoalStatusAfterAmountChange(
  goalId: number,
  userId: number,
  client?: PoolClient
): Promise<void> {
  const r = await runQuery(
    client,
    `SELECT target_amount, current_amount, status FROM financial_goals WHERE id = $1 AND user_id = $2`,
    [goalId, userId]
  );
  ...
  if (current >= target - 0.005 && status === 'ACTIVE') {
    await runQuery(
      client,
      `UPDATE financial_goals SET status = 'COMPLETED', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND user_id = $2`,
      [goalId, userId]
    );
  } else if (current < target - 0.005 && status === 'COMPLETED') {
    await runQuery(
      client,
      `UPDATE financial_goals SET status = 'ACTIVE', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND user_id = $2`,
      [goalId, userId]
    );
  }
}

function parseOptionalAccountId(body: Record<string, unknown>, key: string): number | null {
...
function parseSourceBankUpdate(
  body: Record<string, unknown>,
  previous: number | null
): number | null {
  if (!('sourceBankAccountId' in body)) return previous;
  return parseOptionalAccountId(body, 'sourceBankAccountId');
}

async function validateAccountForGoalCurrency(
  userId: number,
...
function mapMovementRow(row: Record<string, unknown>) {
  return {
    id: row.id,
    goalId: row.goal_id,
    ...
    sourceBankAccountNumber:
      row.source_bank_account_number != null ? String(row.source_bank_account_number) : null,
  };
}
...
export const getGoalMovements = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT m.id, m.goal_id, m.amount, m.note, m.movement_date, m.created_at, m.updated_at,
       ...
       ORDER BY COALESCE(m.movement_date, m.created_at::date) DESC, m.created_at DESC`,
      [userId, goalId]
    );
...
export const addGoalMovement = async (req: AuthRequest, res: Response) => {
  ...
  const { amount, note, movementDate } = req.body as Record<string, unknown>;
  ...
  try {
    ...
    const movementResult = await client.query(
      `INSERT INTO financial_goal_movements (user_id, goal_id, amount, note, movement_date, bank_account_id, source_bank_account_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, goal_id, amount, note, movement_date, created_at, updated_at, bank_account_id, source_bank_account_id`,
      [userId, goalId, numericAmount, note || null, dateStr, snapBankId, sourceBankId]
    );
    ...
    let bankNum: string | null = null;
    if (row.bank_account_id) {
      const bn = await client.query(
        `SELECT bank_name, account_number FROM bank_accounts WHERE id = $1 AND user_id = $2`,
        [row.bank_account_id, userId]
      );
      if (bn.rows.length > 0) {
    ...
    let srcNum: string | null = null;
    if (row.source_bank_account_id) {
      const sn = await client.query(
        `SELECT bank_name, account_number FROM bank_accounts WHERE id = $1 AND user_id = $2`,
        [row.source_bank_account_id, userId]
      );
      if (sn.rows.length > 0) {
...
export const updateGoalMovement = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const movBankId = existingResult.rows[0].bank_account_id as number | null;
    const prevSrcId = existingResult.rows[0].source_bank_account_id as number | null;
    ...
    const noteForDb =
      note === undefined ? existingNote : note === null || note === '' ? null : String(note);
    ...
    const updateResult = await client.query(
      `UPDATE financial_goal_movements
       ...
       RETURNING id, goal_id, amount, note, movement_date, created_at, updated_at, bank_account_id, source_bank_account_id`,
      [numericAmount, noteForDb, dateStr, newSrcId, movementId, goalId, userId]
    );
    ...
    if (row.bank_account_id) {
      ...
      if (bn.rows.length > 0) bankName = String(bn.rows[0].bank_name);
    }
...
export const deleteGoalMovement = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const existingAmount = roundMoney(parseFloat(existingResult.rows[0].amount));
    const movBankId = existingResult.rows[0].bank_account_id as number | null;
    const srcId = existingResult.rows[0].source_bank_account_id as number | null;
