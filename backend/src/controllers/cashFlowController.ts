  ...
  try {
    ...
    fixedIncomeResult.rows.forEach((row) => {
      ...
      if (frequency === 'MONTHLY' && row.receipt_day) {
        ...
        dates = calculateMonthlyRecurringDates(parseInt(row.receipt_day), start, end);
        ...
        dates = calculateRecurringDates(startDate, frequency, end);
      }
    ...
    const recurringExpensesWithDayResult = await query(
      `SELECT amount, currency, payment_day
       FROM expenses
    ...
    recurringExpensesWithDayResult.rows.forEach((row) => {
      ...
      const dates = calculateMonthlyRecurringDates(paymentDay, start, end);
