...
import {
  validateExpenseUpdateForLinkedPayable,
  syncPayablePaymentFromExpense,
  deletePayablePaymentByExpenseId,
} from '../services/accountsPaymentLinkSync';
import { removeExpenseForUser } from '../services/expenseDeletionService';
import { syncVehicleExpenseFromLinkedExpense } from '../services/vehicleExpenseLinkSync';
import {
  FREQUENCY_VALUES,
  normalizeFrequency,
...
function validateExpenseSchedule(
  ...
  if (!frequency || !FREQUENCY_VALUES.includes(frequency)) {
...
function resolveExpenseTaxonomy(merged: Record<string, unknown>): {
  nature: Nature;
  recurrenceType: RecurrenceType;
  frequency: Frequency | null;
} | null {
  const natureVal = merged.nature;
  if (natureVal !== 'fixed' && natureVal !== 'variable') return null;
  const nature = natureVal as Nature;
  const rtRaw = merged.recurrenceType ?? merged.recurrence_type;
  if (rtRaw !== 'recurrent' && rtRaw !== 'non_recurrent') return null;
  const recurrenceType = rtRaw as RecurrenceType;
  let frequency = normalizeFrequency(merged.frequency as string | undefined);
  if (recurrenceType === 'non_recurrent') {
    frequency = null;
  } else {
    if (!frequency || !FREQUENCY_VALUES.includes(frequency)) return null;
  }
...
function amountForVariableMonthlyPeriodRow(
  row: { nature?: string; amount: unknown; period_amount?: unknown | null },
  slotPaidOrReceived: boolean,
  monthlyRec: boolean
): number {
  const base = parseFloat(String(row.amount));
  if (!monthlyRec || row.nature !== 'variable' || !slotPaidOrReceived) return base;
...
export const createExpense = async (req: AuthRequest, res: Response) => {
  ...
  if (!tx) {
    return res.status(400).json({
      message:
        'Se requieren nature (fixed|variable), recurrenceType (recurrent|non_recurrent) y frequency cuando el gasto es recurrente',
    });
  }
  const schedErr = validateExpenseSchedule(
    tx.recurrenceType,
    tx.frequency,
    paymentDay,
    paymentMonth,
  ...
  try {
    ...
    const rowId = result.rows[0].id as number;
    ...
    if (initialPaid && isMonthlyRecurringExpense({ recurrence_type: tx.recurrenceType, frequency: tx.frequency })) {
      ...
      await client.query(
        `UPDATE expenses SET last_paid_month = $1, last_paid_year = $2, updated_at = CURRENT_TIMESTAMP
         WHERE id = $3 AND user_id = $4`,
        [currentMonth, currentYear, rowId, userId]
      );
    }
...
export const updateExpense = async (req: AuthRequest, res: Response) => {
  ...
  const payableErr = await validateExpenseUpdateForLinkedPayable(userId, expenseId, {
    amount,
    currency,
  });
  if (payableErr) {
  ...
  const veLink = await query(`SELECT id FROM vehicle_expenses WHERE linked_expense_id = $1`, [expenseId]);
  ...
  try {
    ...
    await syncVehicleExpenseFromLinkedExpense(client, userId, expenseId, {
...
export const deleteExpense = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const ok = await removeExpenseForUser(userId, expenseId);
    if (!ok) {
      return res.status(404).json({ message: 'Expense not found' });
    }
...
function resolveMonthlyPaymentAmount(
  expense: { nature: string; amount: unknown },
  monthlyRec: boolean,
  actualAmountRaw: unknown
): number {
  const base = parseFloat(String(expense.amount));
  if (!monthlyRec || expense.nature !== 'variable') return base;
...
export const updateExpensePaymentStatus = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId!;
    const expenseId = parseInt(req.params.id);
    ...
    const checkResult = await query(
      `SELECT id, description, nature, recurrence_type, frequency, amount, currency, bank_account_id, last_paid_month, last_paid_year
       FROM expenses WHERE id = $1 AND user_id = $2`,
      [expenseId, userId]
    );
    ...
    const alreadyMarkedPaidThisMonth =
      expense.last_paid_month === currentMonth && expense.last_paid_year === currentYear;

    const applyRecurringBalance = async (paid: boolean, ledgerAmount: number, client?: PoolClient) => {
      if (!monthlyRec || !expense.bank_account_id) return;
      const delta = paid ? -ledgerAmount : ledgerAmount;
      try {
        await applyBalanceDelta(userId, expense.bank_account_id, expense.currency, delta, client, {
          description:
            delta < 0
    ...
    if (monthlyRec) {
      ...
      const reverseLedgerAmount = async (): Promise<number> => {
        const base = parseFloat(String(expense.amount));
        if (expense.nature !== 'variable') return base;
        const v = await getExpensePeriodAmount(userId, expenseId, currentYear, currentMonth);
        ...
        if (isPaid) {
          if (!alreadyMarkedPaidThisMonth) {
            if (expense.nature === 'variable') {
              await upsertExpensePeriodAmount(client, userId, expenseId, currentYear, currentMonth, payAmount);
            }
          ...
          const result = await client.query(
            `UPDATE expenses
             ...
             RETURNING id, description, is_paid, last_paid_month, last_paid_year, updated_at`,
            [isPaid, currentMonth, currentYear, expenseId, userId]
          );
          await client.query('COMMIT');
          res.json({
            success: true,
          ...
          if (alreadyMarkedPaidThisMonth) {
            ...
            if (expense.nature === 'variable') {
              await deleteExpensePeriodAmount(client, userId, expenseId, currentYear, currentMonth);
            }
          }
          const result = await client.query(
            `UPDATE expenses
             ...
             RETURNING id, description, is_paid, updated_at`,
