...
export const createFinancialGoal = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `INSERT INTO financial_goals (user_id, name, description, target_amount, currency, target_date)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, name, description, target_amount, current_amount, currency, target_date, status, created_at, updated_at`,
      [userId, name, description || null, targetAmount, currency, targetDate || null]
    );
  ...
  try {
    ...
    const result = await query(
      `UPDATE financial_goals
       ...
       RETURNING id, name, description, target_amount, current_amount, currency, target_date, status, created_at, updated_at`,
      [name, description, targetAmount, currency, targetDate, currentAmount, status, id, userId]
    );
