...
export const getVehicleExpenses = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT id, expense_type, description, amount, currency, date, mileage_at_expense, category, notes, created_at, updated_at
       FROM vehicle_expenses
       WHERE vehicle_id = $1
...
export const createVehicleExpense = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `INSERT INTO vehicle_expenses (vehicle_id, user_id, expense_type, description, amount, currency, date, mileage_at_expense, category, notes)
  ...
  try {
    ...
    const result = await query(
      `UPDATE vehicle_expenses
       ...
       RETURNING id, expense_type, description, amount, currency, date, mileage_at_expense, category, notes, created_at, updated_at`,
      [expenseType, description, amount, currency, date, mileageAtExpense, category, notes, expenseId, vehicleId]
    );
...
export const deleteVehicleExpense = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await query('DELETE FROM vehicle_expenses WHERE id = $1', [expenseId]);
