...
function googleEvent(id: string, updated: string) {
  return {
    id,
    summary: id,
...
describe('Google Agenda pull pagination', () => {
  ...
  it('collects every page and stores the sync token only after the final page', async () => {
    ...
    mockQuery.mockImplementation(async (sql: string, params?: unknown[]) => {
      if (sql.includes('SELECT sync_token')) return { rows: [{ sync_token: null }] };
      if (sql.includes('UPDATE agenda_provider_connections SET')) writes.push(params || []);
      return { rows: [] };
    });
    mockList
    ...
    expect(result.events.map((event: { externalUid: string }) => event.externalUid)).toEqual([
      'first',
      'second',
    ]);
  ...
  it('clears an invalid sync token and immediately retries once with the configured range', async () => {
    mockQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('SELECT sync_token')) return { rows: [{ sync_token: 'stale-token' }] };
      return { rows: [] };
    });
  ...
  it('emits a tombstone for a mapped cancellation stub without start data', async () => {
    mockQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('SELECT sync_token')) return { rows: [{ sync_token: 'current-token' }] };
