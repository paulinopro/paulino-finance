import { expandAgendaRecurrences, parseAgendaRRule } from './agendaRecurrence';
import type { AgendaItemRow } from './agendaService';

function row(patch: Partial<AgendaItemRow>): AgendaItemRow {
  return {
    id: 1,
    user_id: 1,
    kind: 'EVENT',
...
describe('agenda recurrence expansion', () => {
  it('parses basic RRULE fields', () => {
    expect(parseAgendaRRule('FREQ=DAILY;UNTIL=20260502;INTERVAL=4')).toMatchObject({
      freq: 'DAILY',
      interval: 4,
    });
  });
  ...
  it('expands yearly birthdays into the requested window', () => {
    const rows = expandAgendaRecurrences(
      [
        row({
