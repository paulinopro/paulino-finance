...
const {
  syncAgendaItemToICloudCalDav,
  removeAgendaItemFromICloudCalDav,
} = require('./agendaICloudPushService');
...
describe('iCloud Agenda outbound failure propagation', () => {
  ...
  it('rejects when an event push fails after recording the provider error', async () => {
    ...
    expect(mockRecordICloudError).toHaveBeenCalledWith(
      7,
      expect.stringContaining('CalDAV crear objeto 503')
    );
  });
