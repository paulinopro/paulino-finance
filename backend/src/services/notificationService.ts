...
export const NOTIFICATION_DAILY_CRON = '0 9 * * *';
...
let schedulerConfiguredAtIso: string | null = null;
let dailyNotificationCronTask: ReturnType<typeof cron.schedule> | null = null;

let lastNotificationSweep: NotificationSchedulerSweepStatus | null = null;

function finalizeNotificationSweep(startedAt: string, ok: boolean, errorMessage?: string): void {
...
export function getNotificationSchedulerStatus(): NotificationSchedulerRuntimeStatus {
  const expressionValid = cron.validate(NOTIFICATION_DAILY_CRON);
  const schedulerRegistered = dailyNotificationCronTask != null;
  return {
    cronExpression: NOTIFICATION_DAILY_CRON,
    expressionValid,
    schedulerRegistered,
    ...
    cronTimeNoteEs:
      'Ejecución diaria a las 9:00 según la zona del proceso Node: variable TZ si existe; si no, la zona local del servidor.',
...
function buildCardPaymentTemplateVariables(
  card: Record<string, unknown>,
  days: number,
  localePreference: string | null | undefined,
  pair: UserCurrencyPair
): TemplateVariables {
  const localeTag = localeTagFromUiLanguage(localePreference ?? undefined);
  ...
  let debtText = '';
  if (ct === 'DOP') {
    debtText = formatCurrencyAmount(debtPrimaryRail, primaryIso, localeTag);
  } else if (ct === 'USD') {
    debtText = formatCurrencyAmount(debtSecondaryRail, secondaryIso, localeTag);
  } else {
    debtText = `${formatCurrencyAmount(debtPrimaryRail, primaryIso, localeTag)} / ${formatCurrencyAmount(debtSecondaryRail, secondaryIso, localeTag)}`;
  }
...
export const checkAndSendNotifications = async () => {
  ...
  try {
    ...
    for (const user of usersResult.rows) {
      ...
      const localeTag = localeTagFromUiLanguage(user.locale_preference);
...
<b>Días restantes (recordatorio):</b> {days}`;
              ...
              const message = renderTemplate(
                messageTemplate,
                buildCardPaymentTemplateVariables(card as Record<string, unknown>, days, user.locale_preference, pair)
              );
        ...
        for (const loan of loansResult.rows) {
          ...
          for (const days of daysBefore) {
            ...
            if (
              ...
              const installmentAmountFormatted = formatCurrencyAmount(
                parseFloat(String(loan.installment_amount ?? 0)),
                loanCurrencyIso,
                localeTag
              );
...
<b>Días restantes:</b> {days}`;
              ...
              const message = renderTemplate(messageTemplate, {
                ...
                amount: formatCurrencyAmount(
                  ...
                  String(expense.currency || pair.primary),
                  localeTag
                ),
                currency: '',
...
export const startNotificationScheduler = () => {
  if (!cron.validate(NOTIFICATION_DAILY_CRON)) {
    console.error('[notifications] Expresión cron inválida (no se registró el job):', NOTIFICATION_DAILY_CRON);
    dailyNotificationCronTask = null;
    schedulerConfiguredAtIso = null;
    return;
  }

  dailyNotificationCronTask = cron.schedule(NOTIFICATION_DAILY_CRON, () => {
