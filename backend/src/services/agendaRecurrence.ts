...
function parseRRuleDate(raw: string): Date | null {
  const v = raw.trim();
  if (/^\d{8}$/.test(v)) {
...
export function parseAgendaRRule(raw: string | null | undefined): ParsedRRule | null {
  if (!raw) return null;
  const line = raw
    .split(/\r?\n/)
    .map((x) => x.trim())
  ...
  const freqRaw = parts.get('FREQ') as Frequency | undefined;
  if (!freqRaw || !['DAILY', 'WEEKLY', 'MONTHLY', 'YEARLY'].includes(freqRaw)) return null;
...
function addUtcMonthsClamped(d: Date, months: number): Date {
  const out = new Date(d);
...
function addUtcYearsClamped(d: Date, years: number): Date {
  const out = new Date(d);
  const wantedDay = out.getUTCDate();
  out.setUTCDate(1);
...
function stepOccurrence(anchor: Date, rule: ParsedRRule, index: number): Date {
  if (rule.freq === 'DAILY') return addUtcDays(anchor, index * rule.interval);
  if (rule.freq === 'WEEKLY') return addUtcDays(anchor, index * rule.interval * 7);
  if (rule.freq === 'MONTHLY') return addUtcMonthsClamped(anchor, index * rule.interval);
  return addUtcYearsClamped(anchor, index * rule.interval);
}

function weeklyByDayOccurrences(anchor: Date, rule: ParsedRRule, rangeEnd: Date): Date[] {
  if (rule.freq !== 'WEEKLY' || !rule.byDay?.length) return [];
  const out: Date[] = [];
  ...
  for (let weekIndex = 0; weekIndex < maxIterations; weekIndex += rule.interval) {
    ...
    for (const day of rule.byDay) {
      ...
      if (candidate >= anchor) out.push(candidate);
    }
  }
  return out.sort((a, b) => a.getTime() - b.getTime());
}

export function expandAgendaRecurrences(
  rows: AgendaItemRow[],
  range: { from: Date; to: Date }
): AgendaItemRow[] {
  const expanded: AgendaItemRow[] = [];
