...
const AccountsReceivable: React.FC = () => {
  ...
  const handleReceive = async (id: number) => {
    if (!window.confirm('¿Estás seguro de que deseas marcar esta cuenta como cobrada? Se agregará automáticamente a tus ingresos.')) {
      return;
    }

    try {
      await api.put(`/accounts-receivable/${id}/receive`);
      toast.success('Cuenta marcada como cobrada y agregada a ingresos');
      fetchAccounts();
    } catch (error: any) {
      toast.error('Error al cobrar cuenta');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-dark-400 text-sm sm:text-base">Gestiona tus cuentas por cobrar</p>
        </div>
        <button
          type="button"
          onClick={() => {
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    {account.receivedDate && (
                      <div>
                        <p className="text-dark-400">Fecha de Cobro</p>
                        <p className="text-white font-medium">{new Date(account.receivedDate).toLocaleDateString('es-DO')}</p>
                      </div>
                    )}
                  </div>
                  {account.notes && (
