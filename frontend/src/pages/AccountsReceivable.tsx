...
const AccountsReceivable: React.FC = () => {
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      toast.error(t('toast.accountsReceivable.loadError'));
    } finally {
      setLoading(false);
    }
  }, [statusFilter, t]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.dueDate) {
      toast.error(t('toast.accountsReceivable.dueDateRequired'));
      return;
    }
    try {
      const data = {
      ...
      if (editingAccount) {
        ...
        toast.success(t('toast.accountsReceivable.updated'));
      } else {
        await api.post('/accounts-receivable', data);
        toast.success(t('toast.accountsReceivable.created'));
      }
      ...
      toast.error(error.response?.data?.message || t('toast.accountsReceivable.saveError'));
    }
  };
  ...
  const submitReceive = async (e: React.FormEvent) => {
    ...
    if (!receiveTarget || !receiveDate) {
      toast.error(t('toast.accountsReceivable.incomeDateRequired'));
      return;
    }
    try {
      await api.put(`/accounts-receivable/${receiveTarget.id}/receive`, { paymentDate: receiveDate });
      toast.success(t('toast.accountsReceivable.markedReceived'));
      setShowReceiveModal(false);
      setReceiveTarget(null);
      fetchAccounts();
    } catch (error: any) {
      toast.error(error.response?.data?.message || t('toast.accountsReceivable.markReceivedError'));
    }
  };
  ...
  const submitAbono = async (e: React.FormEvent) => {
    ...
    if (!abonoDate) {
      toast.error(t('toast.accountsReceivable.paymentDateRequired'));
      return;
    }
    try {
      ...
      toast.success(t('toast.accountsReceivable.paymentRegistered'));
      setShowAbonoModal(false);
      setAbonoTarget(null);
      fetchAccounts();
    } catch (error: any) {
      toast.error(error.response?.data?.message || t('toast.accountsReceivable.paymentRegisterError'));
    }
  };
  ...
  const openHistoryModal = async (account: AccountReceivable) => {
    ...
    try {
      ...
      toast.error(t('toast.accountsReceivable.paymentsLoadError'));
    } finally {
  ...
  const submitEditPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('toast.accountsReceivable.paymentUpdatedSync'));
      setPaymentBeingEdited(null);
      await reloadHistoryAndAccounts();
    } catch (error: any) {
      toast.error(error.response?.data?.message || t('toast.accountsReceivable.paymentUpdateError'));
    }
  };
  ...
  const handleDeletePayment = async (p: AccountPaymentRow) => {
    if (!historyTarget) return;
    if (
      !window.confirm(t('confirm.deleteReceivablePayment'))
    ) {
      return;
    }
    try {
      await api.delete(`/accounts-receivable/${historyTarget.id}/payments/${p.id}`);
      toast.success(t('toast.accountsReceivable.paymentDeleted'));
      await reloadHistoryAndAccounts();
    } catch (error: any) {
      toast.error(error.response?.data?.message || t('toast.accountsReceivable.paymentDeleteError'));
    }
  };

  const handleDelete = async (id: number) => {
    if (!window.confirm(t('confirm.deleteReceivableAccount'))) {
      return;
    }

    try {
      await api.delete(`/accounts-receivable/${id}`);
      toast.success(t('toast.accountsReceivable.deleted'));
      fetchAccounts();
    } catch {
      toast.error(t('toast.accountsReceivable.deleteError'));
    }
  };
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'RECEIVED':
        return t('pages.accountsReceivable.statusReceived');
      case 'OVERDUE':
        return t('pages.accountsReceivable.statusOverdue');
      default:
        return t('pages.accountsReceivable.statusPending');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t('pages.accountsReceivable.title')}
        subtitle={t('pages.accountsReceivable.subtitle')}
        actions={
          <div className="flex flex-wrap items-center justify-end gap-2 w-full sm:w-auto">
            ...
            >
              <Plus size={20} />
              {t('pages.accountsReceivable.newAccountButton')}
            </button>
          </div>
        }
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <p className="text-dark-400 text-sm mb-1">
                {t('pages.accountsReceivable.summaryOutstandingToCollect', { currency: primaryCurrency })}
              </p>
              <p className="text-2xl font-bold text-white">
                {fc(receivableSummaryKpis.primaryTotal, primaryCurrency)}
              </p>
            </div>
            <div>
              <p className="text-dark-400 text-sm mb-1">
                {t('pages.accountsReceivable.summaryOutstandingToCollect', { currency: secondaryCurrency })}
              </p>
              <p className="text-2xl font-bold text-white">
                {fc(receivableSummaryKpis.secondaryTotal, secondaryCurrency)}
              </p>
            </div>
            <div>
              <p className="text-dark-400 text-sm mb-1">{t('pages.accountsReceivable.summaryAccountCount')}</p>
      ...
      <div className="card-view">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          >
            <option value="">{t('pages.accountsReceivable.filterAllStatuses')}</option>
            <option value="PENDING">{t('pages.accountsReceivable.statusPending')}</option>
            <option value="RECEIVED">{t('pages.accountsReceivable.statusReceived')}</option>
            <option value="OVERDUE">{t('pages.accountsReceivable.statusOverdue')}</option>
          </select>
        </div>
      </div>
      ...
      {orderedFiltered.length === 0 ? (
        <div className="card-view text-center py-12 sm:py-16">
          <p className="text-dark-400">{t('pages.accountsReceivable.emptyState')}</p>
        </div>
      ) : (
        <>
                  ...
                  <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between xl:gap-3">
                    <div className="order-2 min-w-[min(100%,12rem)] flex-1 space-y-2 xl:order-1 xl:pr-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-dark-600/80 bg-dark-700/50 px-2.5 py-1 text-[0.7rem] font-medium uppercase tracking-wide text-dark-300 sm:text-xs">
                          ...
                          {t('pages.accountsReceivable.badgeToCollect')}
                        </span>
                    ...
                    <div className="order-1 flex w-full shrink-0 flex-wrap items-center justify-end gap-0.5 xl:order-2 xl:w-auto">
                      ...
                      {canAct && (
                        <>
                          <button
                            ...
                            title={t('pages.accountsReceivable.markReceivedTitle')}
                            aria-label={t('pages.accountsReceivable.markReceivedAria')}
                          >
                          ...
                          <button
                            ...
                            title={t('pages.accountsReceivable.addPayment')}
                            aria-label={t('pages.accountsReceivable.addPayment')}
                          >
                      ...
                      <button
                        ...
                        title={t('pages.accountsReceivable.historyTitle')}
                        aria-label={t('pages.accountsReceivable.historyAria')}
                      >
                  ...
                  <div className="mt-4 flex flex-col gap-3 border-t border-dark-700/80 pt-4">
                    {account.status !== 'RECEIVED' && (
                      <div className="space-y-2 rounded-xl border border-dark-600/50 bg-dark-900/25 px-3 py-2.5">
                        <div className="flex flex-wrap items-baseline justify-between gap-2 text-sm">
                          <span className="text-dark-400">{t('pages.accountsReceivable.progressReceivedLabel')}</span>
                          <span className="tabular-nums font-semibold text-white">
                            {fc(totalReceived, account.currency)}
                        ...
                        <p className="text-xs text-dark-500">
                          {t('pages.accountsReceivable.remainingShort')}{' '}
                    ...
                    <div className="metrics-cq">
                      ...
                      <div className="metrics-cell rounded-xl border border-dark-600/60 bg-dark-900/30 px-3 py-2.5 sm:py-3">
                        <p className="text-[0.65rem] font-medium uppercase tracking-wider text-dark-500">
                          {t('pages.accountsReceivable.metricAmount')}
                        </p>
                      ...
                      <div className="metrics-cell rounded-xl border border-dark-600/60 bg-dark-900/30 px-3 py-2.5 sm:py-3">
                        <p className="text-[0.65rem] font-medium uppercase tracking-wider text-dark-500">
                          {t('pages.accountsReceivable.metricDueDate')}
                        </p>
                      ...
                      {account.receivedDate && (
                        <div className="metrics-cell-span-2 rounded-xl border border-dark-600/60 bg-dark-900/30 px-3 py-2.5 sm:py-3">
                          <p className="text-[0.65rem] font-medium uppercase tracking-wider text-dark-500">
                            {t('pages.accountsReceivable.metricLastReceiptClose')}
                          </p>
                    ...
                    {account.notes && (
                      <div className="rounded-xl border border-dark-600/50 bg-dark-900/20 px-3 py-2 text-sm">
                        <span className="text-dark-500">{t('pages.accountsReceivable.notesPrefix')}</span>
          ...
          <TablePagination
            ...
            itemLabel={t('pages.accountsReceivable.itemsLabel')}
            variant="card"
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 id="accounts-receivable-modal-title" className="text-xl font-semibold text-white">
                {editingAccount
                  ? t('pages.accountsReceivable.modalEditTitle')
                  : t('pages.accountsReceivable.modalNewTitle')}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-2">
                  {t('pages.accountsReceivable.fieldDescriptionRequired')}
                </label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-dark-300 mb-2">
                    {t('pages.accountsReceivable.fieldAmountRequired')}
                  </label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="block text-sm font-medium text-dark-300 mb-2">
                    {t('pages.accountsReceivable.fieldCurrencyRequired')}
                  </label>
                  <select
              ...
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-2">
                  {t('pages.accountsReceivable.fieldDueDateRequired')}
                </label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-2">
                  {t('pages.accountsReceivable.fieldCategory')}
                </label>
                <select
                ...
                >
                  <option value="">
                    {categories.length === 0
                      ? t('pages.accountsReceivable.categoryEmptyHint')
                      : t('pages.accountsReceivable.categoryNone')}
                  </option>
              ...
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-2">
                  {t('pages.accountsReceivable.fieldNotes')}
                </label>
                <textarea
              ...
              <div className="flex gap-4">
                <button type="submit" className="btn-primary flex-1">
                  {editingAccount ? t('pages.accountsReceivable.submitUpdate') : t('pages.accountsReceivable.submitCreate')}
                </button>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 id="ar-receive-modal-title" className="text-xl font-semibold text-white">
                {t('pages.accountsReceivable.markReceivedModalTitle')}
              </h2>
            ...
            <p className="text-sm text-dark-400 mb-4">
              {t('pages.accountsReceivable.markReceivedExplanation', {
                amount: fc(receiveTarget.amount - (receiveTarget.totalReceived ?? 0), receiveTarget.currency),
              })}
            </p>
            <form onSubmit={submitReceive} className="space-y-4">
              <div>
                <label className="label">{t('pages.accountsReceivable.fieldIncomeDateRequired')}</label>
                <input type="date" className="input" value={receiveDate} onChange={(e) => setReceiveDate(e.target.value)} required />
              </div>
              <div className="flex gap-4">
                <button type="submit" className="btn-primary flex-1">
                  {t('pages.accountsReceivable.confirmMarkReceived')}
                </button>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 id="ar-abono-modal-title" className="text-xl font-semibold text-white">
                {t('pages.accountsReceivable.addPayment')}
              </h2>
            ...
            <p className="text-sm text-dark-400 mb-4">
              {t('pages.accountsReceivable.maxRemaining', {
                amount: fc(abonoTarget.amount - (abonoTarget.totalReceived ?? 0), abonoTarget.currency),
              })}
            </p>
            <form onSubmit={submitAbono} className="space-y-4">
              <div>
                <label className="label">{t('pages.accountsReceivable.fieldInstallmentAmountRequired')}</label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
              ...
              <div>
                <label className="label">{t('pages.accountsReceivable.destinationAccountOptional')}</label>
                <select
                  className="input w-full"
                  value={abonoBankAccountId}
                  onChange={(e) => setAbonoBankAccountId(e.target.value)}
                >
                  <option value="">{t('pages.accountsReceivable.destinationAccountNoBalance')}</option>
              ...
              <div className="flex gap-4">
                <button type="submit" className="btn-primary flex-1">
                  {t('pages.accountsReceivable.registerInstallment')}
                </button>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-[0.65rem] font-semibold uppercase tracking-wider text-dark-500">
                    {t('pages.accountsReceivable.historyTitle')}
                  </p>
                  ...
                  {!historyLoading && historyPayments.length > 0 && (
                    <p className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-sm text-dark-400">
                      ...
                      <span>
                        {t('pages.accountsReceivable.paymentInstallments', { count: historyPayments.length })}
                      </span>
                    </p>
                  )}
                </div>
            ...
            <div className="mt-4">
              {historyLoading ? (
                ...
                <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-dark-600/60 bg-dark-900/25 px-6 py-12 text-center">
                  ...
                  <p className="max-w-[20rem] text-sm text-dark-400">{t('pages.accountsReceivable.noPayments')}</p>
                </div>
              ) : (
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                        <div className="flex min-w-0 items-start gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-[0.65rem] font-semibold uppercase tracking-wider text-dark-500">
                              {t('pages.accountsReceivable.fieldIncomeDateShort')}
                            </p>
                            <p className="mt-0.5 text-sm font-semibold tabular-nums text-white sm:text-base">
                            ...
                            {(p.bankAccountName || p.bankAccountId != null) && (
                              <p className="mt-1 text-xs text-dark-500">
                                {t('pages.accountsReceivable.destinationLabel')}{' '}
                                {formatBankAccountOptionLabel({
                                  bankName: p.bankAccountName || t('pages.accountsReceivable.bankAccountFallback'),
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-dark-700/50 pt-3 sm:border-t-0 sm:pt-0 sm:pl-2">
                          ...
                          <div className="flex shrink-0 items-center justify-end gap-0.5">
                            <button
                              ...
                              title={t('pages.accountsReceivable.editPayment')}
                              aria-label={t('pages.accountsReceivable.editPayment')}
                            ...
                            <button
                              ...
                              title={t('pages.accountsReceivable.deletePayment')}
                              aria-label={t('pages.accountsReceivable.deletePayment')}
        ...
        <div className="modal-overlay" onClick={() => setPaymentBeingEdited(null)} role="presentation">
          ...
          >
            <h2 id="ar-edit-payment-title" className="text-xl font-semibold text-white mb-4">
              {t('pages.accountsReceivable.editPayment')}
            </h2>
            <form onSubmit={submitEditPayment} className="space-y-4">
              <div>
                <label className="label">{t('pages.accountsReceivable.fieldAmountRequired')}</label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
              ...
              <div>
                ...
                >
                  <option value="">{t('pages.accountsReceivable.sourceAccountNone')}</option>
